...
class UploadIsolateBlock {
  ...
  Future<int> _getUploadedSize([int times = 0]) async {
    final uri = Uri.parse("${this.domain}${Api.APP_FILE_UPLOAD_GET_UPLOADED_SIZE}?md5=${this.info.md5}&_token=${this.info.token}");
  ...
  Future<bool> _uploadByMd5() async {
    final uri = Uri.parse(this.domain + Api.APP_FILE_UPLOAD_BY_MD5 + "?_token=${this.info.token}");
    final client = HttpClient();
    this._client = client;
    try {
      final request = await client.openUrl("POST", uri);
