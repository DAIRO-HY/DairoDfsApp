...
///上传列表详细信息组件
class UCUploadItem extends StatelessWidget {
  ///文件上传信息
  ...
  ///点击回调事件
  UCUploadItem(this.bridge, {required this.selectedIds, required this.onSelectChange});
  ...
  Widget get _uploadedInfo {
    if (this.bridge.dto.state == 10) {
      ...
      return this._context.textSecondarySmall("${this.bridge.dto.uploadedSize.dataSize}/${this.bridge.dto.size.dataSize}");
    }
  }
  ...
  Widget get _stateIcon {
    switch (this.bridge.dto.state) {
      ...
      case 2: //暂停中
      case 3: //出错中
        return Icon(Icons.arrow_circle_up, color: this._context.color.onSurface);
      case 10: //完成
  ...
  void onStateIconClick() {
    ...
    switch (this.bridge.dto.state) {
      case 0: //等待中的任务
        UploadDao.setState(this.bridge.dto.id, 2);
      case 1: //正在上传
        this.bridge.pause();
        return;
      case 2: //暂停中
        UploadDao.setState(this.bridge.dto.id, 0);
        UploadTask.start();
      case 3: //上传失败
        UploadDao.setState(this.bridge.dto.id, 0);
        UploadTask.start();
