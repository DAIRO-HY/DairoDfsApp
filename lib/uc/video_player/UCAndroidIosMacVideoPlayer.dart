...
///Android，IOS，MAC平台的视频播放器
class UCAndroidIosMacVideoPlayer extends UCVideoPlayerBase {
  ///标记播放器是否已经初始化
  var isInit = false;

  ///播放器控制器初始值监听
  final initedValueNotify = ValueNotifier<VideoPlayerController?>(null);
  ...
  Future<void> init(String url) async {
    ...
    if(url.startsWith("http")){
      ...
      //从网络连接播放
      this.videoPlayerController = VideoPlayerController.networkUrl(Uri.parse(url));
    }else{
  ...
  Future<UCProgressInfo> getProgress() async {
    ...
    //当前播放未知
    final current = await this.videoPlayerController!.position ?? Duration(milliseconds: 0);
  ...
  Widget build(BuildContext context) {
    return this.initedValueNotify.build((value) {
      ...
      //return VideoProgressIndicator(value, allowScrubbing: true,);
